...
const colorNames = require('./colorName');
...
module.exports = {
  ...
  injectThemes: function (addBase, config, themes) {
    ...
    // add light themes
    if (config('fuegoui.themes') == false) {
    ...
    // add custom themes
    if (Array.isArray(config('fuegoui.themes'))) {
      config('fuegoui.themes').forEach((item, index) => {
    ...
    let themeOrder = [];
    if (Array.isArray(config('fuegoui.themes'))) {
      config('fuegoui.themes').forEach((theme, index) => {
    ...
    } else if (config('fuegoui.themes') != false) {
      themeOrder = [
        'light',
        'dark',
        'cupcake',
    ...
    themeOrder.forEach((themeName, index) => {
      if (index === 0) {
        ...
        // auto dark
        if (config('fuegoui.darkTheme')) {
          if (
            themeOrder[0] != config('fuegoui.darkTheme') &&
            themeOrder.includes(config('fuegoui.darkTheme'))
          ) {
            addBase({
              ['@media (prefers-color-scheme: dark)']: {
                [':root']:
                  includedThemesObj[
                    `[data-theme=${config('fuegoui.darkTheme')}]`
                  ],
              },
            });
          }
        } else if (config('fuegoui.darkTheme') === false) {
